...
def conv_to_vmap(x, npy_data_in):
    ori = x[:3]
    dir = x[3:6]
    tmin = x[6:7]
    tmax = x[7:8]
...
jit_again = jit(vmap(conv_to_vmap, in_axes=(0, None)))
...
for i in range(int(batch_nb - 1)):
    res = jit_again(x[i * batch_size: (i + 1) * batch_size], npy_data)
    res.block_until_ready()
    tmp_rgb.append(res)

res = jit_again(x[int((batch_nb - 1) * batch_size):], npy_data)
tmp_rgb.append(res)
