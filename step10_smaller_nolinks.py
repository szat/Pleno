...
assert np.sum(npy_links >= 0) == npy_density_data.shape[0]
assert np.sum(npy_links >= 0) == npy_sh_data.shape[0]
assert np.sum(npy_links >= 0) + np.sum(npy_links < 0) == 512**3
...
assert np.sum(npy_links > 0) == npy_density_data.shape[0] - 1
assert np.sum(npy_links > 0) == npy_sh_data.shape[0] - 1
assert np.sum(npy_links >= 0) == 512**3
assert np.sum(npy_links == 0) == 512**3 - npy_density_data.shape[0] + 1

tmp = npy_density_data[npy_links]
assert np.sum(tmp == -999) == 512**3 - npy_density_data.shape[0] + 1
assert np.sum(tmp == -999) + np.sum(tmp != -999) == 512**3

# now 128*128*128
# This is how things are inflated
tmp = tmp[::4, ::4, ::4]
tmp2 = npy_density_data[npy_links[::4, ::4, ::4]]
assert np.all(tmp == tmp2)

npy_data_prev = np.hstack([npy_density_data, npy_sh_data])
npy_data_prev = npy_data_prev[npy_links[::4, ::4, ::4]]
...
assert np.all(npy_data == npy_data_prev)
# so wee see we did all correctly
...
def conv_to_vmap(ori, dir, tmin, tmax, my_sh_in, npy_data_in):
...
jit_again = jit(vmap(conv_to_vmap, in_axes=(0, 0, 0, 0, 0, None)))
